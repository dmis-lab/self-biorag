...
def create_groundness_data(input_data, multi_retrieval=False):
    ...
    for item in input_data:
        ...
        if multi_retrieval is True:
            if "sent_idx" not in item or item["sent_idx"] == 0 or len(item["preceding_sentences"]) == 0:
                processed_data.append({"instruction": PROMPT_DICT["ground_multi_instruction"], "input": PROMPT_DICT["ground_multi_input_wo_preceding"].format_map(input), "output": item["score"], "task": "groundness"})
            else:
