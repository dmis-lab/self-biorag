...
def process_data(input_data, inst_mode, input_mode, split="train", multi_retrieval=False, three_way=False):
    if split == "train":
        ...
        if "target_output" not in input_data:
            input_data["target_output"] = input_data['output']
        """
        if "sent_idx" not in input_data:
            input_data["sent_idx"] = -1
        if "preceding_sentences" not in input_data:
            input_data["preceding_sentences"] = "-1"
        if "evidence" not in input_data:
            input_data["evidence"] = "This is an example of an evidence"
        """
        if multi_retrieval is True and (input_data["sent_idx"] == 0 or "preceding_sentences" not in input_data or type(input_data["preceding_sentences"]) is not str or len(input_data["preceding_sentences"]) == 0):    
            if three_way:
                input = PROMPT_DICT["multi_retrieval" + "_three_way_input" + "_wo_preceding"].format_map(input_data)
            else:
                input = PROMPT_DICT[input_mode + 
                                    "_wo_preceding"].format_map(input_data)
        else:
...
def main():
    ...
    if 'multi_retrieval' in args.task:
    ...
    # len(input_data) // args.batch_size
    for idx in tqdm(range(len(input_data) // args.batch_size)):
        ...
        processed_batch = [process_data(
            item, inst_mode=args.inst_mode, input_mode=args.input_mode, split=args.split, multi_retrieval=multi_retrieval, three_way=args.three_way)[0] for item in batch]
        postprocess_output = [process_data(
            item, inst_mode=args.inst_mode, input_mode=args.input_mode, split=args.split, multi_retrieval=multi_retrieval, three_way=args.three_way)[1] for item in batch]
        ...
        for j, item in enumerate(batch):
            ...
            item["pred"] = pred
            if args.split == "train":
                item["output"] = postprocess_output[j]
                if len(item["pred"]) != "" and item["pred"] == item["output"] or item["pred"] in item["output"] or item["output"] in item["pred"] or item["output"][1:-1] in item["pred"]:
                    item["correct"] = 1.0
                else:
