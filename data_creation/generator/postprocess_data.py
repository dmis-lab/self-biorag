...
def load_all_files(file_paths):
    ...
    for fp in file_paths:
        if fp.endswith(".json") or fp.endswith(".jsonl") or fp.endswith(".json_splitted"):
            data = load_file(fp)
            for item in data:
            ...
            fp_filelist = os.listdir(fp)
            for filename in tqdm(fp_filelist):
                data = load_file(fp+filename)
                for item in data:
                    q_id = item["id"] if "id" in item else item["q_id"]
                    final_results.setdefault(q_id, [])
                    final_results[q_id].append(item)
...
def main():
    ...
    parser.add_argument('--groundness_pred', type=str, nargs="+")
    ...
    reward_groundness = load_all_files(args.groundness_pred)
    ...
    for q_id, instance in tqdm(input_data.items()):
        ...
        if need_retrieval_initial_i == "No Retrieval]":
            need_retrieval_initial_i = "[No Retrieval]"
        ...
        if need_retrieval_initial_i == "[No Retrieval]":
            ...
            if random.random() > 0.6:
                ...
                # processed_data.append({"instruction": instruction, "output": processed_output, "input": input, "id": q_id, "dataset_name": dataset_name})
                processed_data.append({"instruction": instruction + input, "output": processed_output, "input": "", "id": q_id, "dataset_name": dataset_name})
            ...
            if q_id not in decisions_need_retrieval_multi:
                ...
                print(q_id)
                continue
            if q_id not in splitted_output_data:
                print ("{} missing splitted output data".format(q_id))
                continue
            if q_id not in retrieval_data:
                print ("{} missing retrieval data".format(q_id))
                continue
            decisions_need_retrieval_multi_i = decisions_need_retrieval_multi[q_id]
            for item in decisions_need_retrieval_multi_i:
                if item["sent_idx"] == -1:
                    item["sent_idx"] = 0
            ...
            for sent_idx, sentence in enumerate(splitted_output_sentences):
                ...
                if need_retrieval_decision_token == "[Retrieval]":
                    ...
                    try:
                        ...
                        try:
                            ctx = retrieved_results[sent_idx]["evidence"]
                        except:
                            continue
            ...
            if  len(postprocess(output)) > 0:
                # processed_data.append({"instruction": instruction, "output": output, "input": input, "id": q_id, "dataset_name": dataset_name})
                processed_data.append({"instruction": instruction+input, "output": output, "input": "", "id": q_id, "dataset_name": dataset_name})
